...
export interface GameState {
  ...
  gameOver: boolean; // Added: gameOver to GameState
  winningTeam: number | null; // Added: winningTeam to GameState
  ...
  teamNames: string[]; // Added: Display names for teams
  // Note: Player-specific umbrella state (isUmbrellaOpen, umbrellaAngle) is part of each Player object in the players array
}
...
export function createServer(port: number): {
  ...
  let lastTick: number | null = null; // Declare lastTick here
  const socketClientId = new Map<WebSocket, string>(); // Maps WebSocket connection to the first clientId associated with it
  const clientAssociatedControllerIds = new Map<WebSocket, Set<string>>(); // Maps WebSocket to a set of controller clientIds
  const disconnectedPlayerStates = new Map<string, Partial<Player>>(); // Stores state of disconnected players
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'disconnect':
            ...
            const disconnectedControllerId = (msg as MsgDisconnect).clientId;
            ...
            if (playerToRemove) {
              ...
              console.log(
                `Player ${playerToRemove.playerName} (ID: ${disconnectedControllerId}) disconnected, state saved.`
              );
            }
          ...
          case 'level':
            ...
            wss.clients.forEach((client) => {
              if (client.readyState === WebSocket.OPEN) {
                client.send(
                  JSON.stringify({ type: 'level', levelId: msg.levelId })
                );
              }
            });
            break;
      ...
      if (associatedIds) {
        associatedIds.forEach((controllerId) => {
          ...
          if (playerWhoLeft) {
            ...
            console.log(
              `Player ${playerWhoLeft.playerName} (ID: ${controllerId}) disconnected due to WebSocket close, state saved.`
            );
          }
        });
      }
